...
function buildContinuationContext(previous, legacyContext = '', language = 'es') {
  ...
  if (edgeFigure) {
    blocks.push(isEs
      ...
      : `EDGE_FIGURE_DETECTED (внутренняя метка, НЕ произносить пользователю): «${edgeFigure.slice(0, 500)}». Это уже выявленная краевая фигура/функция, а не новая тема. Возможная защитная или ограничивающая функция — внутренняя методологическая гипотеза: не объявляй её клиенту и не задавай наводящий вопрос о защите. Исследуй феноменологически, что фигура говорит, запрещает, боится, избегает или не допускает, и что происходит у перехода. Не переноси эту функцию на телесный симптом. Не возвращайся к рассказу сна, картированию с нуля, первичному/вторичному вопросу или выбору нового фокуса, если пользователь сам явно не меняет тему.`);
  }
