...
export function buildBodyStageInstruction(stage, language = "es") {
  ...
  if (stage.stage === "awaiting_body_signal") {
    ...
    return es
      ? `\n\n🔴 BODY: SEÑAL CORPORAL\nHaz exactamente una pregunta: «${q}»${boundary}`
  ...
  if (stage.stage === "body_small_u") {
    ...
    return es
      ? `\n\n🔴 BODY: EXPERIENCIA HABITUAL DE LA SEÑAL\nExplora con detalle cómo vive habitualmente la persona esta señal: localización, cualidad, efecto en su actividad o conducta cotidiana, patrón temporal y contexto observable. No busques una causa.\nYa se han recogido: ${(stage.body_primary_dimensions || []).join(", ") || "solo la señal inicial"}.\nHaz EXACTAMENTE una pregunta siguiente: «${q}»\nSi una imagen X ya apareció espontáneamente, recuérdala, pero no la despliegues todavía hasta comprender suficientemente la experiencia habitual.${boundary}`
  ...
  if (stage.stage === "body_amplify_signal") {
    return es
      ? `\n\n🟠 BODY: HACER MÁS CLARA LA SEÑAL → IMAGEN X\nLa experiencia habitual ya está suficientemente descrita. Haz la señal corporal solo un poco más clara, dentro de una intensidad tolerable, y deja que pueda transformarse en una IMAGEN / FIGURA / FORMA X. No inventes la imagen por la persona. Haz una sola pregunta experimental, por ejemplo: «Si esta sensación se hiciera un poco más clara, ¿qué imagen o forma empezaría a aparecer? ¿A qué se parece?»\n${channels}\nIMPORTANTE: un canal no sustituye a la imagen; describe cómo la persona percibe o despliega una imagen que ya está emergiendo.${boundary}`
  ...
  if (stage.stage === "body_identify_x") {
    return es
      ? `\n\n🟠 BODY: IMAGEN X PRESENTE → EXPERIMENTO DE IDENTIFICACIÓN\nX es una imagen o figura que porta una cualidad todavía poco habitual; aún no la trates como una conclusión sobre la persona. Invítala brevemente y con permiso a probar cómo sería ser esa imagen concreta, utilizando sus propias palabras. Haz una sola pregunta sobre la experiencia inmediata desde X.\n${channels}\nNo impongas lo visual: una misma imagen puede vivirse mediante cualquiera de los seis canales — verse, sonar, respirarse, moverse, aparecer en relación o sentirse en el entorno.${boundary}`
  ...
  if (stage.stage === "body_unfold_x") {
    const detected = (stage.x_channels || []).length ? (es ? ` Ya han aparecido estos canales: ${(stage.x_channels || []).join(", ")}.` : ` Уже проявились каналы: ${(stage.x_channels || []).join(", ")}.`) : "";
    return es
      ? `\n\n🟠 BODY: DESPLEGAR LA IMAGEN X A TRAVÉS DE LOS CANALES\nLa persona ya ha entrado en la imagen X. Deja que esa experiencia se despliegue. ${channels}${detected}\nSigue el canal que realmente está apareciendo y observa cambios hacia otro solo si surgen de forma natural. La PERSONA COMO X puede moverse, sonar, respirar, mirar, relacionarse o experimentar el entorno. Haz exactamente una pregunta experimental. No pases todavía al significado ni a la aplicación en la vida.${boundary}`
  ...
  if (stage.stage === "body_discover_big_u") {
    return es
      ? `\n\n🟢 BODY: DESCUBRIR LA NUEVA CUALIDAD DE SÍ\nLa imagen X ya ha sido suficientemente vivida. Pregunta ahora cómo cambia la persona cuando esa energía está disponible: qué cualidades, estado o forma de estar aparecen. No sugieras cualidades ni interpretes. Internamente, este cambio de la persona —no la imagen ni el canal— es el material secundario. Haz una sola pregunta.${boundary}`
  ...
  if (stage.stage === "body_stabilize_big_u") {
    return es
      ? `\n\n🟢 BODY: ESTABILIZAR LA CUALIDAD EMERGENTE\nLa nueva cualidad ya apareció. No corras todavía a analizar la vida. Ayuda a permanecer unos instantes con este estado: cómo se siente en el cuerpo, postura, respiración y presencia, y qué ayuda a reconocerlo. Usa las cualidades exactas nombradas por la persona. Haz una sola pregunta de estabilización.${boundary}`
  ...
  if (stage.stage === "body_integrate_big_u") {
    return es
      ? `\n\n🔵 BODY: INTEGRAR LA CUALIDAD EMERGENTE EN LA VIDA\nIntegra ahora únicamente las cualidades que la persona ya ha nombrado: cómo podrían tener un lugar en su vida, qué pequeño cambio o acción sería coherente, o cómo se ven desde ahí problemas, relaciones o decisiones conocidas. Haz una pregunta cada vez. No vuelvas a buscar una causa del síntoma.${boundary}`
