...
export function buildContinuationPrompt({ rows, terms, messages, language, systemPrompt, memoriesBlock = "", startedAt, processContext = {} }) {
  ...
  const edgeRule = structuredContext.edge_figures.length
    ? (es
      ? "\nCONTEXTO ESTRUCTURADO: ya apareció al menos una figura/voz crítica o prohibitiva. No la pierdas al continuar y NO la llames «figura del borde» ante la persona. Si sigue activa y la persona no eligió otro foco, prioriza *_continue_edge. Su posible función protectora o limitante es una HIPÓTESIS METODOLÓGICA INTERNA. Primero respeta cómo la vive la persona (miedo, rechazo, enfado, asco, incomodidad) y explora qué dice, prohíbe, teme, evita o no permite. Solo después de suficiente contacto puedes invitar a mirarla con cierta distancia y desde otra perspectiva: qué hace de hecho en su vida, qué sostiene, evita, limita o qué posibles ventajas puede tener. No preguntes directamente «¿qué te protege?» y no presupongas que hay ventajas. Nunca atribuyas esa función a un síntoma corporal. Una pregunta cada vez y usando las palabras de la persona.\n"
      : "\nСТРУКТУРИРОВАННЫЙ КОНТЕКСТ: уже появилась как минимум одна критическая/запрещающая фигура или голос. Не теряй её при продолжении и НЕ называй клиенту «краевой фигурой». Если она остаётся активной и человек не выбрал другой фокус, приоритетно используй *_continue_edge. Возможная защитная или ограничивающая функция — ВНУТРЕННЯЯ МЕТОДОЛОГИЧЕСКАЯ ГИПОТЕЗА. Сначала уважай, как человек реально переживает эту фигуру (страх, отвращение, злость, неприязнь, дискомфорт), и исследуй, что она говорит, запрещает, боится, избегает или не допускает. Только после достаточного контакта можно мягко предложить посмотреть на неё чуть со стороны и другим взглядом: что она фактически делает в жизни человека, что поддерживает, предотвращает, ограничивает или какие возможные плюсы у неё есть. Не спрашивай в лоб «что она тебя защищает?» и не предполагай, что плюсы обязательно есть. Никогда не переноси эту функцию на телесный симптом. Один вопрос за раз, словами человека.\n")
    : "";
