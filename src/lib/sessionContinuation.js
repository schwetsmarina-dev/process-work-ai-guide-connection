...
export function buildContinuationPrompt({ rows, terms, messages, language, systemPrompt, memoriesBlock = "", startedAt, processContext = {} }) {
  ...
  const edgeRule = structuredContext.edge_figures.length
    ? (es
      ? "\nCONTEXTO ESTRUCTURADO: ya apareció al menos una figura del borde/voz crítica o prohibitiva. No la pierdas al continuar. Si sigue activa en el último material y la persona no eligió otro foco, prioriza el paso *_continue_edge. La posible función protectora o limitante de la figura es una HIPÓTESIS METODOLÓGICA INTERNA, no algo que se anuncia al cliente. No preguntes de forma dirigida «¿qué te protege?» ni atribuyas protección a un síntoma. Explora fenomenológicamente qué dice la figura, qué prohíbe, qué teme, qué evita o qué no permite, una pregunta cada vez y usando las palabras de la persona. No inventes su historia.\n"
      : "\nСТРУКТУРИРОВАННЫЙ КОНТЕКСТ: уже появилась как минимум одна краевая фигура/критический или запрещающий голос. Не теряй её при продолжении. Если она остаётся активной в последнем материале и человек не выбрал другой фокус, приоритетно используй шаг *_continue_edge. Возможная защитная или ограничивающая функция фигуры — ВНУТРЕННЯЯ МЕТОДОЛОГИЧЕСКАЯ ГИПОТЕЗА, а не то, что сообщается клиенту. Не задавай наводящий вопрос «что она тебя защищает?» и не приписывай защитную функцию симптому. Феноменологически исследуй, что фигура говорит, запрещает, боится, избегает или не допускает, по одному вопросу и словами человека. Не придумывай историю за человека.\n")
    : "";
