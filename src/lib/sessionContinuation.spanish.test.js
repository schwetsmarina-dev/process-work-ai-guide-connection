...
describe("Spanish methodology reaching the AI gateway", () => {
  it.each(["body", "dream", "conflict", "journaling"])("%s sends Spanish rules, all steps and term applications", async mode => {
    ...
    expect(prompt).toContain("HIPÓTESIS METODOLÓGICA INTERNA");
    expect(prompt).toContain("qué hace de hecho en su vida");
    expect(prompt).not.toContain("La figura del borde protege algo importante");
