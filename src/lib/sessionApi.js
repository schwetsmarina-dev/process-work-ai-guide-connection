...
function buildContinuation(previous, legacyContext = "", modeId = "", language = "es") {
  ...
  if (edgeFigure) {
    contextParts.push(isEs
      ? `EDGE_FIGURE_DETECTED — ETIQUETA PROFESIONAL INTERNA, NO DECIRLA AL USUARIO. En el trabajo anterior ya apareció una figura interna crítica o prohibitiva: «${edgeFigure.slice(0, 600)}». Considérala un borde ya identificado. La posible función protectora o limitante es una hipótesis metodológica interna: no se anuncia ni se pregunta de forma dirigida. Continúa explorando fenomenológicamente qué dice la figura, qué prohíbe, qué teme, qué evita o qué no permite y qué ocurre en la transición. No atribuyas esa función a un síntoma corporal. No vuelvas a un sueño nuevo, al mapeo primario/secundario desde cero ni a elegir otro foco salvo que el usuario cambie explícitamente de tema.`
      : `EDGE_FIGURE_DETECTED — ВНУТРЕННЯЯ ПРОФЕССИОНАЛЬНАЯ МЕТКА, НЕ ПРОИЗНОСИТЬ ПОЛЬЗОВАТЕЛЮ. В предыдущей работе уже появилась запрещающая/критикующая внутренняя фигура: «${edgeFigure.slice(0, 600)}». Считай это уже выявленным краем. Возможная защитная или ограничивающая функция — внутренняя методологическая гипотеза: не объявляй её клиенту и не задавай наводящий вопрос о защите. Исследуй феноменологически, что фигура говорит, запрещает, боится, избегает или не допускает и что происходит у перехода. Не переноси эту функцию на телесный симптом. Не возвращайся к новому сну, первичному/вторичному картированию или выбору нового фокуса, если пользователь сам не меняет тему.`);
  }
