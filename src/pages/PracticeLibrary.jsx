...
export default function PracticeLibrary() {
  ...
  const visibleRows = useMemo(() => rows
    // Spanish rows used to disappear permanently when author_public_es was
    // empty: the UI hid them before the localization pass could ever help.
    // A real named author in the canonical author field is a safe fallback;
    // generic provenance-only rows remain hidden by hasNamedAuthor().
    .filter((x) => hasNamedAuthor(lang === "es" ? (x.author_public_es || x.author) : x.author))
  ...
  const practices = useMemo(() => {
    ...
    return localizedRows.filter((x) => {
      ...
      const haystack = lang === "es"
        ? [x.title_es, x.author_public_es || publicAuthor(x.author), x.purpose_es, ...(x.steps_es || []), ...(x.search_tags_es || []), ...(x.term_keys || [])].join(" ").toLocaleLowerCase()
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-muted-foreground">
                      <span className="inline-flex items-center gap-1"><UserRound className="w-3.5 h-3.5" />{c.author}: {lang === "es" ? (exercise.author_public_es || publicAuthor(exercise.author)) : publicAuthor(exercise.author)}</span>
                    </div>
                  </div>
